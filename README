...
About the project
The project consists of a C# class library where I keep my WCF services and a Web Application that is the actual Web REST API.
...
The first returns a list of all the people. The second takes a parameter called Keyword that gets automatically deserialized into an instance of PersonLookupInfo. I did it this way because I wanted to show you how you can actually pass several parameters and have them deserialized automatically as properties of a single data structure.

The GetPeople() method is decorated with a WebGet attribute (System.ServiceModel.Web) and the UriTemplate is "GetPeople".
The GetPeopleWithPOST() method is decorated with a WebInvoke attribute which can be used with other http methods, like POST or even PUT etc. In this case, I'm using POST by specifying the Method parameter.
Also, when using WebInvoke, you want to specify the BodyStyle to Bare. I'm not really sure how this works, but I experience some problems with the others and did not have time to look further into it.
...
In my case, I did not want ASP.NET to do the redirection so a good solution I found on the web was to use an HttpModule that intercepts 302 redirection and returns instead a 401 (Unauthorized).
...
In my case I decided I was going to use an HttpHandler to handle incoming Login request. This will return a Cookie if the Login was successful.
...
But then I had to add an exception for the Login.ashx file which I want to be accessible by anonymous users:
...
Finally, the <system.serviceModel> is configured just to handle default end points. I'm NOT going to use the config to setup each WCF service, because configuring WCF sucks. I've been working with WCF for many years and I still think this is one of the down sides of WCF.
...
This step might be confusing for beginners. So again, first you build a request to call the Login.ashx and get a cookie, which will come in the Set-Cookie header.
...
Any comments/suggestions are appreciated. If you need help, please do not hesitate to contact me.
